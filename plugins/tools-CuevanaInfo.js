// If you reuse this, please keep the credits.
import axios from 'axios';
import * as cheerio from 'cheerio';

const handler = async (m, { conn, text, prefix: usedPrefix, command }) => {
  if (!text) return m.reply(`🎬 *Provide a Cuevana3 link.*\nExample: ${usedPrefix + command} https://wwv.cuevana3.eu/ver-pelicula/cars`);
  ...
  try {
    ...
    let infoMsg = `🎬 *${title}*\n\n`;
    infoMsg += `📋 *Information:*\n${info}\n\n`;
    infoMsg += `📖 *Synopsis:* ${sinopsis || 'Not available'}\n\n`;
    infoMsg += `🌐 *Watch online:* ${onlineLink}\n\n`;
    
    if (downloads.length > 0) {
      infoMsg += `📥 *Download links:*\n`;
      downloads.forEach(d => {
        infoMsg += `▸ *Server:* ${d.server}\n`;
        infoMsg += `▸ *Language:* ${d.language}\n`;
        infoMsg += `▸ *Quality:* ${d.quality}\n`;
        infoMsg += `🔗 ${d.link}\n\n`;
      });
    } else {
      infoMsg += `⚠️ *No download links available.*`;
    }
    ...
    console.error('Error in getInfoCuevana:', e);
    m.reply('❌ *Failed to fetch the information. Check the link or try again later.*');
  }
};
handler.tags = ['tools']
...
async function extraerRedireccionDesdeHTML(url) {
  try {
    ...
    console.error("Error in extraerRedireccionDesdeHTML:", error.message);
    return null;
  }
}
