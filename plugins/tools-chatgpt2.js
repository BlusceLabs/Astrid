...
let handler = async (m, { conn, text, command }) => {
  ...
  try {  
    ...
    if (command === 'delmemoryia' || command === 'borrarmemoriaai') {
      ...
      return m.reply('🗑️ Conversation memory cleared successfully.\n\nI will no longer remember our previous chats.');
    }
...
async function luminsesi(prompt, sender, contextLogic = '') {
  ...
  const logic = contextLogic || 'You are a bot named Astrid, always polite and helpful.';
  ...
  try {
    ...
    return '⚠️ An error occurred while contacting the API.';
  }
}
