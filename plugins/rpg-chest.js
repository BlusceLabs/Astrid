...
const handler = async (m, {isPrems, conn}) => {
  ...
  const astrid = Math.floor(Math.random() * 4000);
  const expp = Math.floor(Math.random() * 5000);

  global.db.data.users[m.sender].limit += dia;
  global.db.data.users[m.sender].money += astrid;
...
${tradutor.texto2[7]}`;
  ...
  await conn.sendFile(m.chat, img, 'astrid.jpg', texto, fkontak);
