...
const handler = async (m, { conn, isOwner, usedPrefix, command, text }) => {
 ...
 await m.reply('*[❌] Failed to create a temporary email right now.*');
 }
}
 ...
 await m.reply('*[❌] Failed to read the mailbox right now.*');
 }
}
 if (feature == 'delete') {
 ...
 try {
  ...
  await m.reply('*[❌] Failed to delete the temporary email session.*');
   }
  }  
 }
};
