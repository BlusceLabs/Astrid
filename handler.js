...
autoread: false,
    ...
    const commandCooldown = global.db.data.users[m.sender]?.cooldown || {};
    const now = Date.now();
    const cooldownTime = plugin.cooldown || 3000;
    if (!isPrems && !isROwner && !isOwner && commandCooldown[name] && now - commandCooldown[name] < cooldownTime) {
      const remaining = Math.ceil((commandCooldown[name] + cooldownTime - now) / 1000);
      return m.reply(`⏱️ Wait _${remaining}s_ before using this command again.`);
    }
    global.db.data.users[m.sender].cooldown = global.db.data.users[m.sender].cooldown || {};
    global.db.data.users[m.sender].cooldown[name] = now;

    const userRateLimit = global.db.data.users[m.sender].rateLimit || { count: 0, resetTime: 0 };
    const rateWindow = 60000;
    const maxCommands = 20;
    if (now > userRateLimit.resetTime) {
      global.db.data.users[m.sender].rateLimit = { count: 1, resetTime: now + rateWindow };
    } else {
      global.db.data.users[m.sender].rateLimit.count++;
      if (!isPrems && !isROwner && !isOwner && global.db.data.users[m.sender].rateLimit.count > maxCommands) {
        const waitSec = Math.ceil((userRateLimit.resetTime - now) / 1000);
        return m.reply(`⚠️ Rate limit exceeded. Wait _${waitSec}s_ before sending more commands.`);
      }
    }
    ...
    for (const name in global.plugins) {
      ...
      if (typeof plugin.all === 'function') {
        try {
          ...
          /*fetch('https://astridbot.cloud:2083/error', {
            method: 'POST',
...
${tradutor.texto1[1]} ${messageNumber}/3
 ...
 ${tradutor.texto1[4]}`.trim();
    ...
    if (m) {
      ...
      if (m.plugin) {
        const now = +new Date;
        const pluginName = m.plugin.replace('.js', '');
        if (m.plugin in stats) {
          ...
          if (!isNumber(stat.errors)) {
            stat.errors = 0;
          }
          if (!Array.isArray(stat.users)) {
            stat.users = [];
          }
        } else {
          stat = stats[m.plugin] = {
        ...
        if (m.error == null) {
          ...
          stat.errors = (stat.errors || 0) + 1;
        }
        if (m.sender && !stat.users.includes(m.sender)) {
          stat.users.push(m.sender);
        }
        const userStats = global.db.data.users[m.sender]?.cmdStats || {};
        userStats[pluginName] = (userStats[pluginName] || 0) + 1;
        userStats.total = (userStats.total || 0) + 1;
        global.db.data.users[m.sender].cmdStats = userStats;
      }
    }
