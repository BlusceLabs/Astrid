...
const sharedPhoneValidator = new PhoneValidator();

function resolveKnownJidSync(jid, conn) {
    try {
        if (!jid || typeof jid !== "string" || !jid.endsWith("@lid")) return jid;

        const lidKey = jid.replace("@lid", "");
        const cachedInfo =
            conn?.lid?.getUserInfo?.(lidKey) ||
            global?.conn?.lid?.getUserInfo?.(lidKey);
        if (cachedInfo?.jid && !cachedInfo.jid.endsWith("@lid")) {
            return cachedInfo.jid;
        }

        const phoneDetection =
            conn?.lid?.detectPhoneInLid?.(lidKey) ||
            global?.conn?.lid?.detectPhoneInLid?.(lidKey) ||
            sharedPhoneValidator.detectPhoneInLid(lidKey);
        if (phoneDetection?.isPhone && phoneDetection?.jid) {
            return phoneDetection.jid;
        }

        return jid;
    } catch (error) {
        console.error("Error resolving known JID:", error);
        return jid;
    }
}
...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    const resolveOutboundJid = async (jid, options = {}) => {
        try {
            if (!jid || typeof jid !== "string" || !jid.endsWith("@lid")) return jid;

            const knownJid = resolveKnownJidSync(jid, conn);
            if (knownJid && !knownJid.endsWith("@lid")) {
                return knownJid;
            }

            const quotedGroupId =
                options?.quoted?.chat?.endsWith?.("@g.us")
                    ? options.quoted.chat
                    : options?.quoted?.key?.remoteJid?.endsWith?.("@g.us")
            ...
            console.error("Error resolving outbound JID:", error);
            return jid;
        }
    };
...
END:VCARD
        ...
        sendMessage: {
            async value(jid, content, options = {}) {
                const resolvedJid = await resolveOutboundJid(jid, options);
                return rawSendMessage(resolvedJid, content, options);
            },
        },
...
// Fix 2025 - @hoodDevs - LID Resolved
export function serialize() {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        isBaileys: {
            get() {
                try {
                    ...
                    const baileysStarts = ['NJX-', 'Lyru-', 'META-', 'EvoGlobalBot-', 'FizzxyTheGreat-', 'BAE5', '3EB0', 'B24E', '8SCO', 'SUKI', 'ASTRID-'];
                    const hasKnownPrefix = baileysStarts.some(prefix => messageId.startsWith(prefix));
		            const isSukiPattern = /^SUKI[A-F0-9]+$/.test(messageId);
					const isAstridPattern = /^ASTRID[A-F0-9]+$/.test(messageId);
                    return isAstridPattern || isSukiPattern || hasKnownPrefix || false;
                } catch (e) {
            ...
            get() {
                try {
                    ...
                    const rawJid =
                        ...
                        "";
                    const decodedJid = safeDecodeJid(rawJid, this.conn);
                    return decodedJid?.endsWith?.("@lid") && !decodedJid.endsWith("@g.us")
                        ? resolveKnownJidSync(decodedJid, this.conn)
                        : decodedJid;
                } catch (e) {
        ...
        sender: {
            get() {
                const decodedJid = this.conn?.decodeJid(
                ...
                return resolveKnownJidSync(decodedJid, this.conn);
            },
            enumerable: true,
        },
        fromMe: {
                            ...
                            sender: {
                                get() {
                                    try {
                                        ...
                                        return resolveKnownJidSync(parsedJid, self.conn);
                                    } catch (e) {
